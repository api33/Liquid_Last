...
def funcmain(image_add):
    ...
    plt.ylabel('Count')
    plt.xlabel('Gray')
    plt.savefig('pic_sav/' + image_add.split('/')[1].split('.')[0] + '.png')
...
def Main_All_top_four():
    ...
    # funcmain('picture3/'+file_names[2])
    ...
    double_list_str = str(Alltop_four_peak)

    # 打开文件以写入模式
    with open('my_double_list.txt', 'w') as file:
        # 将双层列表字符串写入文件
        file.write(double_list_str)
    plt.figure()

    # 在循环中的每次迭代前清除之前的图形
    plt.clf()
    for i in range(len(Alltop_four_peak)):
        y=Alltop_four_peak[i]
        x=[i]*4
